...
const buildOperationNameMap = (preOps: PreOpsData[] = []): Record<string, string> => {
  const map: Record<string, string> = {};
  preOps.forEach((p) => {
    const opIds = Array.isArray(p.operation_id) ? p.operation_id : [];
    const history = p.surgical_history ?? "";
    opIds.forEach((id) => {
      if (id) map[String(id)] = history;
    });
  });
  return map;
};

export default function SurgicalComponent({ surgical_data, pre_ops_data }: Props) {
  const operationNameMap = buildOperationNameMap(pre_ops_data);

  if (surgical_data.length === 0) {
    return (
      <div className="bg-gradient-to-br from-blue-50 to-blue-100/60 rounded-3xl shadow-lg border border-blue-200/50 p-10 text-center text-gray-500 italic">
        No surgical information available.
      </div>
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-green-50 rounded-3xl  backdrop-blur-lg p-6 space-y-6 overflow-x-auto">
      <Table className="rounded-l min-w-[900px] md:min-w-[1000px] lg:min-w-[1200px]">
        ...
        <TableBody>
          {surgical_data.map((rec, idx) => {
            const displayName =
              rec.operation_id?.map((id) => operationNameMap[id] || `Operation #${id}`).join(", ") ||
              "Unknown Operation";
            ...
            return (
              ...
              >
                ...
                <TableCell >{fmtDate(rec.operation_date)}</TableCell>
                <TableCell className="w-48 break-words">{rec.procedure_notes ?? "--"}</TableCell>
                <TableCell >{rec.challenges_during_surgery ?? "--"}</TableCell>
                <TableCell >{rec.complications ?? "--"}</TableCell>
                <TableCell >{rec.nature_of_anesthesia ?? "--"}</TableCell>
                <TableCell >{rec.post_operative_recovery ?? "--"}</TableCell>
                <TableCell className="w-48 break-words">{rec.post_operative_recovery_notes ?? rec.remarks ?? "--"}</TableCell>
              </TableRow>
            );
          })}
        </TableBody>
        ...
        <TableFooter>
          <TableRow>
            <TableCell colSpan={8} className="text-right text-blue-900 ">
              Total Records: {surgical_data.length}
            </TableCell>
          </TableRow>
        </TableFooter>
      </Table>
